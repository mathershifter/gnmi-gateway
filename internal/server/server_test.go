...
func TestGNMISubscribeUnresponsiveClient(t *testing.T) {
	...
	go client1.Subscribe(ctx, q1, gnmiclient.Type) //nolint:errcheck
...
func TestGNMICoalescedDupCount(t *testing.T) {
	...
	ctx := t.Context()
	go c.Subscribe(ctx, q, gnmiclient.Type) //nolint:errcheck
...
func TestSubscriptionLimit(t *testing.T) {
	...
	for range totalQueries {
		c := client.BaseClient{}
		go c.Subscribe(context.Background(), q, gnmiclient.Type) //nolint:errcheck
	}
