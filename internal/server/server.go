...
func (c *matchClient) Update(n any) {
...
func (s *Server) processSubscription(c *streamClient) {
	...
	if !c.sr.GetSubscribe().GetUpdatesOnly() {
		for _, subscription := range c.sr.GetSubscribe().Subscription {
			...
			err = s.c.Query(c.target, fullPath, func(path []string, l *ctree.Leaf, val any) error {
...
func MakeSubscribeResponse(n any, dup uint32) (*pb.SubscribeResponse, error) {
